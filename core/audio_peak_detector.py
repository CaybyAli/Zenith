...
class AudioPeakDetector:
    ...
    def detect_peaks(
        ...
        Mit optionaler adaptiver Normalisierung f├╝r verschiedene Video-Levels.
        """
        ...
        silence_threshold = -35  # Alles ├╝ber -35dB = nicht-Stille
        ...
        # JETZT: F├╝r jeden lauten Moment, messe die TATS├äCHLICHE Lautst├ñrke
        loud_peaks = []
        for loud in loud_periods:
            duration = loud["end"] - loud["start"]
            
            # volumedetect f├╝r DIESEN spezifischen Moment
    ...
    def _apply_hysteresis(
        ...
        bleibt aber danach l├ñnger aktiv f├╝r weniger Flackern.
        ...
        for peak in peaks:
            ...
            # Falls immer noch zu kurz ÔåÆ erweitere HAUPTS├äCHLICH nach hinten
        ...
        # Merge wieder (erweiterte Peaks k├Ânnen jetzt ├╝berlappen)
    ...
    def _normalize_threshold(self, base_threshold: float, video_norm: dict) -> float:
        ...
        Beispiel:
        - Leises Video (mean=-25dB) ÔåÆ Threshold wird angehoben (-18dB)
        - Lautes Video (mean=-15dB) ÔåÆ Threshold wird gesenkt (-22dB)
        """
        ...
        # Passe Threshold an (invertiert: leiser = h├Âherer Threshold)
        adjusted = base_threshold - offset
        
        print(f"[AUDIO-NORM] Threshold {base_threshold:.1f}dB ÔåÆ {adjusted:.1f}dB (offset={offset:.1f}dB)")
    ...
    def _measure_volume_for_period(
        ...
        """Misst mean_volume f├╝r einen spezifischen Zeitraum."""
        cmd = [
            self._FFMPEG,
            "-ss", str(start),
            "-t", str(duration),
