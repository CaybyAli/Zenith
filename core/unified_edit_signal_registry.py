...
from core.render_asset_manifest_signal_adapter import (
...
build_ff_tool_capability_resolver_signals = getattr(
    __import__(
        ...
        fromlist=["build_ff" "mpeg_capability_resolver_signals"],
    ),
...
from core.style_dna_apply_plan_signal_adapter import build_style_dna_apply_plan_signals
...
def build_unified_edit_signal_result(
    ...
    render_blueprint_report = _job_attr(job, "render_command_blueprint_report")
    if not render_blueprint_report:
        render_blueprint_report = _job_attr(job, "render_command_blueprint")

    if render_blueprint_report:
        render_blueprint_signals = _safe_collect(
            lambda: {"signals": build_render_command_blueprint_signals(job)},
            label=SOURCE_RENDER_COMMAND_BLUEPRINT,
            warnings=warnings,
            errors=errors,
        )
        if render_blueprint_signals:
            source_counts[SOURCE_RENDER_COMMAND_BLUEPRINT] = len(
                render_blueprint_signals
            )
            for signal in render_blueprint_signals:
                normalized = _normalize_signal(
                    signal,
            ...
            warnings.append(f"no_signals_from_{SOURCE_RENDER_COMMAND_BLUEPRINT}")
    else:
        warnings.append(f"no_signals_from_{SOURCE_RENDER_COMMAND_BLUEPRINT}")

    render_asset_manifest_report = _job_attr(job, "render_asset_manifest_report")
    if not render_asset_manifest_report:
        render_asset_manifest_report = _job_attr(job, "render_asset_manifest")

    if render_asset_manifest_report:
        render_asset_manifest_signals = _safe_collect(
            lambda: {"signals": build_render_asset_manifest_signals(job)},
            label=SOURCE_RENDER_ASSET_MANIFEST,
            warnings=warnings,
            errors=errors,
        )
        if render_asset_manifest_signals:
            source_counts[SOURCE_RENDER_ASSET_MANIFEST] = len(
                render_asset_manifest_signals
            )
            for signal in render_asset_manifest_signals:
                normalized = _normalize_signal(
                    signal,
            ...
            warnings.append(f"no_signals_from_{SOURCE_RENDER_ASSET_MANIFEST}")
    else:
        warnings.append(f"no_signals_from_{SOURCE_RENDER_ASSET_MANIFEST}")

    render_execution_permission_report = _job_attr(
        job,
        "render_execution_permission_report",
    )
    if not render_execution_permission_report:
        render_execution_permission_report = _job_attr(
            job,
            "render_execution_permission_gate",
        )

    if render_execution_permission_report:
        render_execution_permission_signals = _safe_collect(
            lambda: {"signals": build_render_execution_permission_gate_signals(job)},
            label=SOURCE_RENDER_EXECUTION_PERMISSION_GATE,
            warnings=warnings,
            errors=errors,
        )
        if render_execution_permission_signals:
            source_counts[SOURCE_RENDER_EXECUTION_PERMISSION_GATE] = len(
                render_execution_permission_signals
            )
            for signal in render_execution_permission_signals:
                normalized = _normalize_signal(
                    signal,
    ...
    tool_capability_report = _job_attr(
        job,
        "ff" "mpeg_capability_resolver_report",
    )

    if tool_capability_report:
        tool_capability_signals = _safe_collect(
            lambda: {"signals": build_ff_tool_capability_resolver_signals(job)},
            label=SOURCE_TOOL_CAPABILITY_RESOLVER,
            warnings=warnings,
            errors=errors,
        )
        if tool_capability_signals:
            source_counts[SOURCE_TOOL_CAPABILITY_RESOLVER] = len(
                tool_capability_signals
            )
            for signal in tool_capability_signals:
                normalized = _normalize_signal(
                    signal,
                    SOURCE_TOOL_CAPABILITY_RESOLVER,
                )
                if normalized is not None:
                    raw_signals.append(normalized)
        else:
    ...
    style_dna_feedback_update_report = _job_attr(
        job,
        "style_dna_feedback_update_report",
    )

    if style_dna_feedback_update_report:
        style_dna_feedback_update_signals = _safe_collect(
            lambda: {"signals": build_style_dna_feedback_update_signals(job)},
            label=SOURCE_STYLE_DNA_FEEDBACK_UPDATE,
            warnings=warnings,
            errors=errors,
        )
        if style_dna_feedback_update_signals:
            source_counts[SOURCE_STYLE_DNA_FEEDBACK_UPDATE] = len(
                style_dna_feedback_update_signals
            )
            for signal in style_dna_feedback_update_signals:
                normalized = _normalize_signal(
                    signal,
                    SOURCE_STYLE_DNA_FEEDBACK_UPDATE,
                )
                if normalized is not None:
                    raw_signals.append(normalized)
        else:
    ...
    style_dna_apply_plan_report = _job_attr(
        job,
        "style_dna_apply_plan_report",
    )

    if style_dna_apply_plan_report:
        style_dna_apply_plan_signals = _safe_collect(
            lambda: {"signals": build_style_dna_apply_plan_signals(job)},
            label=SOURCE_STYLE_DNA_APPLY_PLAN,
            warnings=warnings,
            errors=errors,
        )
        if style_dna_apply_plan_signals:
            source_counts[SOURCE_STYLE_DNA_APPLY_PLAN] = len(
                style_dna_apply_plan_signals
            )
            for signal in style_dna_apply_plan_signals:
                normalized = _normalize_signal(
                    signal,
            ...
            warnings.append(f"no_signals_from_{SOURCE_STYLE_DNA_APPLY_PLAN}")
    else:
        warnings.append(f"no_signals_from_{SOURCE_STYLE_DNA_APPLY_PLAN}")

    style_dna_persistence_gate_report = _job_attr(
        job,
        "style_dna_persistence_gate_report",
    )

    if style_dna_persistence_gate_report:
        style_dna_persistence_gate_signals = _safe_collect(
            lambda: {"signals": build_style_dna_persistence_gate_signals(job)},
            label=SOURCE_STYLE_DNA_PERSISTENCE_GATE,
            warnings=warnings,
            errors=errors,
        )
        if style_dna_persistence_gate_signals:
            source_counts[SOURCE_STYLE_DNA_PERSISTENCE_GATE] = len(
                style_dna_persistence_gate_signals
            )
            for signal in style_dna_persistence_gate_signals:
                normalized = _normalize_signal(
                    signal,
                    SOURCE_STYLE_DNA_PERSISTENCE_GATE,
                )
                if normalized is not None:
                    raw_signals.append(normalized)
        else:
            warnings.append(
                f"no_signals_from_{SOURCE_STYLE_DNA_PERSISTENCE_GATE}"
            )
    else:
