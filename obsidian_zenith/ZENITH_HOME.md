# ZENITH HOME

Stand: 2026-06-06

## Start hier

Wenn du neu in diesem Projekt bist, lies in dieser Reihenfolge:

1. [[CURRENT_TRUTH]]
2. [[Phase_Status]]
3. [[Safety_Index]]
4. [[Script_Index]]
5. [[P5L_Runbook]]
6. [[Phase5_5_Opening_Gate]]
7. [[NEXT_PROMPT]]

## Aktueller Stand

- Phase 5: 100% / DONE / FINAL-GO.
- P5-L: 100% / CLOSED.
- P5-L6.5 5F P5-L Close: DONE.
- Runtime Learning Gate: later / locked.
- Phase 5.5 Musik: 5% / Opening-Gate.
- Musik-Build: noch nicht gestartet.

## Wichtigste Regel

Nicht einfach weiterbauen.

Vor jeder Arbeit:
- HEAD und `git status --short` pruefen.
- Scope lesen.
- Nur erlaubte Dateien anfassen.
- Keine Reports committen.
- Kein `git add .` und kein `git add -A`.

## Schnellnavigation

### Aktuelle Wahrheit
- [[CURRENT_TRUTH]]
- [[Phase_Status]]
- [[GO_NO_GO_Log]]
- [[NEXT_PROMPT]]

### Orientierung
- [[Architecture_Map]]
- [[Script_Index]]
- [[Safety_Index]]
- [[P5L_Runbook]]

### Phase 5
- [[Phase5_Endcriteria_Audit]]

### P5-L
- [[Learning_Opening_Gate]]
- [[Learning_Safety_Rules]]
- [[Learning_Backlog]]
- [[Learning_Run_Log]]
- [[Codex_Audit_Log]]
- [[P5L_Close_Report]]
- [[Runtime_Learning_Gate]]

### Phase 5.5 Musik
- [[Phase5_5_Opening_Gate]]
- [[Phase5_5_Safety_Rules]]
- [[Phase5_5_Backlog]]
- [[Phase5_5_Run_Log]]

### Historie und Webseite
- [[Progress_Log]]
- [[Webseite_Checkliste]]
