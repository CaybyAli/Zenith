...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=995:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeIntakeManager:
    def create_gaming_job(
        ...
        *,
        ...
        return Job(
            job_id="job_process_gaming_channel_dynamic_plan_export_smoke",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["process gaming channel dynamic plan export smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for dynamic export smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision):
        output_path = os.path.join(
            "tmp",
            "process_gaming_channel_dynamic_plan_export_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Process Gaming Channel Dynamic Plan Export Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Process gaming channel dynamic plan export smoke description",
            hashtags=["#zenith", "#dynamic", "#longform"],
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "process_gaming_channel_dynamic_plan_export_smoke")
    export_path = os.path.join(
        "exports",
        ChannelType.GAMING_UNCUT.value,
        "job_process_gaming_channel_dynamic_plan_export_smoke",
    )
    ...
    assert result["job"].job_id == "job_process_gaming_channel_dynamic_plan_export_smoke"
    ...
    loaded_plan = DynamicEditPlanRepository().load_plan(export_path)

    assert loaded_plan is not None
    assert len(loaded_plan.reaction_moments) >= 1
    assert len(loaded_plan.zoom_instructions) >= 1
    assert len(loaded_plan.pacing_hints) >= 1
    assert os.path.exists(os.path.join(export_path, "dynamic_edit_plan.json"))

    print("PROCESS GAMING CHANNEL DYNAMIC PLAN EXPORT SMOKE TEST PASSED")
    print(
        {
            "reaction_moments": len(loaded_plan.reaction_moments),
            "zoom_instructions": len(loaded_plan.zoom_instructions),
            "pacing_hints": len(loaded_plan.pacing_hints),
