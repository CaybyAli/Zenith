...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=990:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeIntakeManager:
    def create_gaming_job(
        ...
        *,
        ...
        return Job(
            job_id="job_process_gaming_channel_reframe_export_smoke",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["process gaming channel reframe export smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for reframe export smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision):
        output_path = os.path.join(
            "tmp",
            "process_gaming_channel_reframe_export_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Process Gaming Channel Reframe Export Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Process gaming channel reframe export smoke description",
            hashtags=["#zenith", "#reframe", "#longform"],
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "process_gaming_channel_reframe_export_smoke")
    export_path = os.path.join(
        "exports",
        ChannelType.GAMING_UNCUT.value,
        "job_process_gaming_channel_reframe_export_smoke",
    )
    ...
    assert result["job"].job_id == "job_process_gaming_channel_reframe_export_smoke"
    assert result["pipeline"]["edit_timeline"] is not None
    assert result["pipeline"]["reframe_plan"] is not None

    loaded_plan = ReframePlanRepository().load_plan(export_path)

    assert loaded_plan is not None
    assert len(loaded_plan.instructions) >= 1
    assert loaded_plan.primary_target_aspect_ratio == "16:9"
    assert loaded_plan.secondary_target_aspect_ratio == "9:16"
    assert os.path.exists(os.path.join(export_path, "reframe_plan.json"))

    print("PROCESS GAMING CHANNEL REFRAME EXPORT SMOKE TEST PASSED")
    print(
        {
            "instructions": len(loaded_plan.instructions),
            "plan_score": loaded_plan.plan_score,
            "primary_target": loaded_plan.primary_target_aspect_ratio,
            "secondary_target": loaded_plan.secondary_target_aspect_ratio,
        }
    )
