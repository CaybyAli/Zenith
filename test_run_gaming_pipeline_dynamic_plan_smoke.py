...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_dynamic_plan_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=960:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["dynamic plan pipeline smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for dynamic plan smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_dynamic_plan_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Dynamic Plan Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline dynamic plan smoke description",
            hashtags=["#zenith", "#dynamic"],
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_dynamic_plan_smoke")
    ...
    assert len(result["dynamic_edit_plan"].reaction_moments) >= 1
    assert len(result["dynamic_edit_plan"].zoom_instructions) >= 1
    assert len(result["dynamic_edit_plan"].pacing_hints) >= 1
    assert result["dynamic_edit_plan"].plan_score >= 0.50

    print("RUN GAMING PIPELINE DYNAMIC PLAN SMOKE TEST PASSED")
    print(
        {
            "reaction_moments": len(result["dynamic_edit_plan"].reaction_moments),
            "zoom_instructions": len(result["dynamic_edit_plan"].zoom_instructions),
            "pacing_hints": len(result["dynamic_edit_plan"].pacing_hints),
            "plan_score": result["dynamic_edit_plan"].plan_score,
        }
    )
