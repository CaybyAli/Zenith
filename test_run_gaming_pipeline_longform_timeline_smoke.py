...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_longform_timeline_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=930:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["longform timeline pipeline smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for longform timeline smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_longform_timeline_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Longform Timeline Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline longform timeline smoke description",
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_longform_timeline_smoke")
    ...
    assert len(result["edit_timeline"].selected_segments) >= 1
    assert result["edit_timeline"].hook_segment_id is not None
    assert result["edit_timeline"].payoff_segment_id is not None
    assert result["edit_timeline"].timeline_score >= 0.45

    print("RUN GAMING PIPELINE LONGFORM TIMELINE SMOKE TEST PASSED")
    print(
        {
            "segments": len(result["edit_timeline"].selected_segments),
            "timeline_score": result["edit_timeline"].timeline_score,
            "target_duration": result["edit_timeline"].target_duration,
        }
    )
