...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_music_application_main_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1060:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["music application main smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for music application main smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_music_application_main_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Music Application Main Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline music application main smoke description",
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_music_application_main_smoke")
    ...
    assert len(result["local_music_selections"]) >= 1
    assert result["music_application_plan"] is not None
    assert len(result["music_application_plan"].instructions) == len(result["local_music_selections"])
    assert all(
        instruction.channel_type == "gaming_main"
        for instruction in result["music_application_plan"].instructions
    )
    assert result["music_application_plan"].application_score > 0.70

    print("RUN GAMING PIPELINE MUSIC APPLICATION MAIN SMOKE TEST PASSED")
    print(
        {
            "instructions": len(result["music_application_plan"].instructions),
            "asset_ids": [instruction.asset_id for instruction in result["music_application_plan"].instructions],
            "application_score": result["music_application_plan"].application_score,
        }
    )
