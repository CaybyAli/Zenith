...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_music_apply_partial_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1160:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["music apply partial smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for music apply partial smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None, music_application_plan=None):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_music_apply_partial_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Music Apply Partial Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline music apply partial smoke description",
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_music_apply_partial_smoke")
    ...
    assert result["music_apply_result"]["partial_apply_used"] is True
    assert result["music_apply_result"]["applied_music_segment_count"] == 1
    assert result["music_apply_result"]["missing_music_segment_count"] == 1
    assert result["music_apply_result"]["music_application_asset_ids"] == ["music_001"]
    assert result["music_apply_result"]["missing_music_asset_ids"] == ["music_004"]
    assert os.path.exists(result["final_video_path"])

    print("RUN GAMING PIPELINE MUSIC APPLY PARTIAL SMOKE TEST PASSED")
    print(
        {
            "reason": result["music_apply_result"]["reason"],
            "applied_music_segment_count": result["music_apply_result"]["applied_music_segment_count"],
            "missing_music_segment_count": result["music_apply_result"]["missing_music_segment_count"],
            "music_application_asset_ids": result["music_apply_result"]["music_application_asset_ids"],
            "missing_music_asset_ids": result["music_apply_result"]["missing_music_asset_ids"],
        }
    )
