...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_reframe_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=940:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for reframe smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_reframe_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Reframe Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline reframe smoke description",
            hashtags=["#zenith", "#reframe"],
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_reframe_smoke")
    ...
    assert len(result["reframe_plan"].instructions) == len(result["edit_timeline"].selected_segments)
    assert result["reframe_plan"].plan_score >= 0.60
    assert result["reframe_plan"].primary_target_aspect_ratio == "16:9"
    assert result["reframe_plan"].secondary_target_aspect_ratio == "9:16"

    print("RUN GAMING PIPELINE REFRAME SMOKE TEST PASSED")
    print(
        {
            "timeline_segments": len(result["edit_timeline"].selected_segments),
            "instructions": len(result["reframe_plan"].instructions),
            "plan_score": result["reframe_plan"].plan_score,
        }
    )
