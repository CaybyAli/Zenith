...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_uncut_music_disabled_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1030:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["uncut music disabled smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for uncut music disabled smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_uncut_music_disabled_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Uncut Music Disabled Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline uncut music disabled smoke description",
            hashtags=["#zenith", "#uncut", "#nomusic"],
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_uncut_music_disabled_smoke")
    ...
    assert result["music_cue_plan"] is None
    assert result["final_edit_package"] is not None
    assert result["final_edit_package"]["music_enabled"] is False
    assert result["final_edit_package"]["audio_cues"] == 0
    assert result["final_edit_package"]["audio_mix_instructions"] == 0
    assert "music" not in result["final_edit_package"]["used_layers"]

    print("RUN GAMING PIPELINE UNCUT MUSIC DISABLED SMOKE TEST PASSED")
    print(result["final_edit_package"])
