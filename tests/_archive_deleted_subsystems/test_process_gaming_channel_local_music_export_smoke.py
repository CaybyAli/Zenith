...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1050:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeIntakeManager:
    def create_gaming_job(
        ...
        *,
        ...
        return Job(
            job_id="job_process_gaming_channel_local_music_export_smoke",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["process gaming channel local music export smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for local music export smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None):
        output_path = os.path.join(
            "tmp",
            "process_gaming_channel_local_music_export_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Process Gaming Channel Local Music Export Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Process gaming channel local music export smoke description",
            hashtags=["#zenith", "#main", "#music"],
        )
...
class FakeAutopublishGate:
    def decide(self, job, validator_result):
        return PublishDecision(
            ...
            reason="local music export smoke",
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "process_gaming_channel_local_music_export_smoke")
    export_path = os.path.join(
        "exports",
        ChannelType.GAMING_MAIN.value,
        "job_process_gaming_channel_local_music_export_smoke",
    )
    ...
    LocalMusicAsset(
        asset_id="music_002",
        channel_type="gaming_main",
        title="Main Tension Bed",
        file_path="assets/audio/gaming_main/music/main_tension_bed.mp3",
        duration_seconds=88.0,
        energy_level=0.74,
        mood_tags=["tense"],
        cue_kinds=["tension_bed", "build_up"],
        notes=[],
    ),
    LocalMusicAsset(
        asset_id="music_003",
    ...
    assert result["job"].job_id == "job_process_gaming_channel_local_music_export_smoke"
    assert result["pipeline"]["music_cue_plan"] is not None
    assert len(result["pipeline"]["local_music_selections"]) >= 1
    assert result["pipeline"]["final_edit_package"] is not None
    assert result["pipeline"]["final_edit_package"]["music_enabled"] is True

    loaded = LocalMusicSelectionRepository().load_selections(export_path)

    assert len(loaded) >= 1
    assert all(selection.channel_type == "gaming_main" for selection in loaded)
    assert len({selection.asset_id for selection in loaded}) == len(loaded)
    assert os.path.exists(os.path.join(export_path, "local_music_selections.json"))

    print("PROCESS GAMING CHANNEL LOCAL MUSIC EXPORT SMOKE TEST PASSED")
    print(
        {
            "selections": len(loaded),
            "asset_ids": [selection.asset_id for selection in loaded],
            "music_enabled": result["pipeline"]["final_edit_package"]["music_enabled"],
        }
    )
