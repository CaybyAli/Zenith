...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1070:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeIntakeManager:
    def create_gaming_job(
        ...
        *,
        ...
        return Job(
            job_id="job_process_gaming_channel_music_application_export_smoke",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["process gaming channel music application export smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for music application export smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None):
        output_path = os.path.join(
            "tmp",
            "process_gaming_channel_music_application_export_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Process Gaming Channel Music Application Export Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Process gaming channel music application export smoke description",
            hashtags=["#zenith", "#main", "#musicapply"],
        )
...
class FakeAutopublishGate:
    def decide(self, job, validator_result):
        return PublishDecision(
            ...
            reason="music application export smoke",
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "process_gaming_channel_music_application_export_smoke")
    export_path = os.path.join(
        "exports",
        ChannelType.GAMING_MAIN.value,
        "job_process_gaming_channel_music_application_export_smoke",
    )
    ...
    assert result["pipeline"]["music_application_plan"] is not None

    loaded_plan = MusicApplicationPlanRepository().load_plan(export_path)
    assert loaded_plan is not None
    assert len(loaded_plan.instructions) >= 1
    assert all(instruction.channel_type == "gaming_main" for instruction in loaded_plan.instructions)
    assert os.path.exists(os.path.join(export_path, "music_application_plan.json"))

    print("PROCESS GAMING CHANNEL MUSIC APPLICATION EXPORT SMOKE TEST PASSED")
    print(
        {
            "instructions": len(loaded_plan.instructions),
            "asset_ids": [instruction.asset_id for instruction in loaded_plan.instructions],
            "application_score": loaded_plan.application_score,
        }
    )
