...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1170:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeIntakeManager:
    def create_gaming_job(
        ...
        *,
        ...
        return Job(
            job_id="job_process_gaming_channel_music_apply_partial_export_smoke",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["process gaming channel music apply partial export smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for music apply partial export smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None, music_application_plan=None):
        output_path = os.path.join(
            "tmp",
            "process_gaming_channel_music_apply_partial_export_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Process Gaming Channel Music Apply Partial Export Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Process gaming channel music apply partial export smoke description",
            hashtags=["#zenith", "#main", "#musicpartial"],
        )
...
class FakeAutopublishGate:
    def decide(self, job, validator_result):
        return PublishDecision(
            ...
            reason="music apply partial export smoke",
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "process_gaming_channel_music_apply_partial_export_smoke")
    export_path = os.path.join(
        "exports",
        ChannelType.GAMING_MAIN.value,
        "job_process_gaming_channel_music_apply_partial_export_smoke",
    )
    ...
    missing_calm_path = os.path.join(catalog_music_dir, "main_calm_bed_missing.mp3")
    ...
    assets = [
        ...
        LocalMusicAsset(
            asset_id="music_004",
            channel_type="gaming_main",
            title="Main Calm Bed Missing",
            file_path=missing_calm_path,
            duration_seconds=8.0,
            energy_level=0.28,
            mood_tags=["calm"],
    ...
    music_apply_result = result["pipeline"]["music_apply_result"]

    assert music_apply_result["music_applied"] is True
    assert music_apply_result["partial_apply_used"] is True
    assert music_apply_result["reason"] == "partial_timeline_segments_applied"
    assert music_apply_result["music_apply_timeline_source"] == "pipeline"
    assert music_apply_result["applied_music_segment_count"] == 1
    assert music_apply_result["missing_music_segment_count"] == 1
    assert music_apply_result["music_application_asset_ids"] == ["music_001"]
    assert music_apply_result["missing_music_asset_ids"] == ["music_004"]

    assert result["pipeline"]["music_apply_timeline"].timeline_id == loaded_timeline.timeline_id
    assert music_apply_result["music_apply_timeline_id"] == loaded_timeline.timeline_id
    ...
    assert result["pipeline"]["final_video_path"].endswith("_music_applied.mp4")
    assert result["pipeline"]["publish_packages"][0].video_path.endswith("_music_applied.mp4")

    print("PROCESS GAMING CHANNEL MUSIC APPLY PARTIAL EXPORT SMOKE TEST PASSED")
    print(
        {
            "timeline_id": loaded_timeline.timeline_id,
            "applied_music_segment_count": music_apply_result["applied_music_segment_count"],
            "missing_music_segment_count": music_apply_result["missing_music_segment_count"],
            "music_application_asset_ids": music_apply_result["music_application_asset_ids"],
            "missing_music_asset_ids": music_apply_result["missing_music_asset_ids"],
            "partial_apply_used": music_apply_result["partial_apply_used"],
        }
    )
