...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1005:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeIntakeManager:
    def create_gaming_job(
        ...
        *,
        ...
        return Job(
            job_id="job_process_gaming_channel_music_cue_export_smoke",
...
class FakeAnalyzer:
    def analyze(self, job):
        return AnalysisResult(
            ...
            notes=["process gaming channel music cue export smoke"],
        )
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for music cue export smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision):
        output_path = os.path.join(
            "tmp",
            "process_gaming_channel_music_cue_export_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Process Gaming Channel Music Cue Export Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Process gaming channel music cue export smoke description",
            hashtags=["#zenith", "#music", "#longform"],
        )
...
def main() -> None:
    test_dir = os.path.join("tmp", "process_gaming_channel_music_cue_export_smoke")
    export_path = os.path.join(
        "exports",
        ChannelType.GAMING_UNCUT.value,
        "job_process_gaming_channel_music_cue_export_smoke",
    )
    ...
    assert result["job"].job_id == "job_process_gaming_channel_music_cue_export_smoke"
    assert result["pipeline"]["edit_timeline"] is not None
    assert result["pipeline"]["reframe_plan"] is not None
    assert result["pipeline"]["dynamic_edit_plan"] is not None
    assert result["pipeline"]["music_cue_plan"] is not None
    assert result["pipeline"]["final_edit_package"] is not None

    loaded_plan = MusicCuePlanRepository().load_plan(export_path)

    assert loaded_plan is not None
    assert len(loaded_plan.audio_cues) >= 1
    assert len(loaded_plan.audio_mix_instructions) >= 1
    assert os.path.exists(os.path.join(export_path, "music_cue_plan.json"))
    assert result["pipeline"]["final_edit_package"]["audio_cues"] >= 1
    assert result["pipeline"]["final_edit_package"]["integration_score"] >= 0.70

    print("PROCESS GAMING CHANNEL MUSIC CUE EXPORT SMOKE TEST PASSED")
    print(
        {
            "audio_cues": len(loaded_plan.audio_cues),
            "mix_instructions": len(loaded_plan.audio_mix_instructions),
            "plan_score": loaded_plan.plan_score,
            "integration_score": result["pipeline"]["final_edit_package"]["integration_score"],
        }
    )
