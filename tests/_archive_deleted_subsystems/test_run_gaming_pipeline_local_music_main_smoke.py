...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_local_music_main_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1040:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for local music main smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_local_music_main_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Local Music Main Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline local music main smoke description",
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_local_music_main_smoke")
    ...
    assert len(result["local_music_selections"]) == len(result["music_cue_plan"].audio_cues)
    assert all(selection.channel_type == "gaming_main" for selection in result["local_music_selections"])
    assert len({selection.asset_id for selection in result["local_music_selections"]}) == len(result["local_music_selections"])
    assert result["final_edit_package"]["music_enabled"] is True

    print("RUN GAMING PIPELINE LOCAL MUSIC MAIN SMOKE TEST PASSED")
    print(
        {
            "music_cue_plan_audio_cues": len(result["music_cue_plan"].audio_cues),
            "local_music_selections": len(result["local_music_selections"]),
            "asset_ids": [selection.asset_id for selection in result["local_music_selections"]],
        }
    )
