...
def build_job(video_path: str) -> Job:
    return Job(
        job_id="job_run_gaming_pipeline_music_apply_main_smoke",
...
def create_sample_video(output_path: str) -> None:
    ...
    cmd = [
        ...
        "lavfi",
        "-i",
        "sine=frequency=1120:sample_rate=44100:duration=14",
        "-c:v",
        "libx264",
        "-c:a",
        "aac",
...
class FakeShortsEngine:
    def decide(self, job, analysis_result, edit_decision):
        return SimpleNamespace(
            ...
            shorts_count=0,
            selected_segments=[],
            decision_reason="not needed for music apply main smoke",
        )
...
class FakeRenderer:
    def render(self, job, edit_decision, final_edit_package=None, music_application_plan=None):
        output_path = os.path.join(
            "tmp",
            "run_gaming_pipeline_music_apply_main_smoke",
            "rendered_video.mp4",
        )
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
...
class FakeTitleGenerator:
    def generate(self, job):
        return TitlePackage(
            job_id=job.job_id,
            primary_title="Run Gaming Pipeline Music Apply Main Smoke",
...
class FakeMetadataGenerator:
    def generate(self, job, title_package):
        return MetadataPackage(
            job_id=job.job_id,
            description="Run gaming pipeline music apply main smoke description",
...
def main() -> None:
    test_dir = os.path.join("tmp", "run_gaming_pipeline_music_apply_main_smoke")
    ...
    assert result["music_apply_result"]["music_apply_segment_count"] == 2
    assert result["music_apply_result"]["applied_music_segment_count"] == 2
    assert result["music_apply_result"]["music_application_asset_ids"] == ["music_001", "music_004"]
    assert os.path.exists(result["final_video_path"])
    assert result["final_video_path"].endswith("_music_applied.mp4")

    print("RUN GAMING PIPELINE MUSIC APPLY MAIN SMOKE TEST PASSED")
    print(
        {
            "music_applied": result["music_apply_result"]["music_applied"],
            "music_apply_segment_count": result["music_apply_result"]["music_apply_segment_count"],
