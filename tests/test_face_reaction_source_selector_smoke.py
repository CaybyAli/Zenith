...
from models.face_reaction_source import (
    FACE_SELECTED_TYPE_FALLBACK_VIDEO_PATH,
    FACE_SELECTED_TYPE_NONE,
    ...
    FACE_SOURCE_STATUS_SELECTED_FALLBACK,
...
def test_raw_video_path_is_preferred(tmp_path):
    ...
    selection = select_face_reaction_source(job)

    assert selection.status == FACE_SOURCE_STATUS_SELECTED
    assert selection.selected_path == str(raw_video)
    assert selection.selected_type == FACE_SELECTED_TYPE_RAW_VIDEO_PATH
...
def test_preprocessing_manifest_source_path_is_used_as_fallback(tmp_path):
    ...
    assert selection.status == FACE_SOURCE_STATUS_SELECTED_FALLBACK
    assert selection.selected_path == str(source_video)
    assert selection.selected_type == FACE_SELECTED_TYPE_PREPROCESSING_SOURCE_PATH
    assert selection.source_exists is True
...
def test_fallback_video_path_is_used(tmp_path):
    ...
    assert selection.selected_type == FACE_SELECTED_TYPE_FALLBACK_VIDEO_PATH
    assert selection.source_exists is True
    assert selection.metadata["fallback_field"] == "video_path"


def test_missing_raw_source_blocks_face_reaction(tmp_path):
    ...
    assert selection.status == FACE_SOURCE_STATUS_BLOCKED_MISSING_VIDEO_SOURCE
...
def test_missing_all_sources_skips_face_reaction():
    job = SimpleNamespace(
        raw_video_path=None,
        preprocessing_manifest={},
        input_file=None,
    ...
    assert selection.status == FACE_SOURCE_STATUS_SKIPPED_NO_VIDEO_SOURCE
    assert selection.selected_path is None
    assert selection.selected_type == FACE_SELECTED_TYPE_NONE
    assert selection.source_exists is False
    assert selection.warnings


def test_face_reaction_source_selection_roundtrip():
    selection = FaceReactionSourceSelection(
        status=FACE_SOURCE_STATUS_SELECTED,
        selected_path="video.mp4",
        selected_type=FACE_SELECTED_TYPE_RAW_VIDEO_PATH,
        checked_sources=[
            {
                "source_type": FACE_SELECTED_TYPE_RAW_VIDEO_PATH,
                "path": "video.mp4",
                "exists": True,
                "status": "exists",
            }
    ...
    restored = FaceReactionSourceSelection.from_dict(selection.to_dict())

    assert restored.to_dict() == selection.to_dict()


def test_new_face_reaction_source_files_do_not_have_bom():
    files = [
        REPO_ROOT / "models" / "face_reaction_source.py",
        REPO_ROOT / "core" / "face_reaction_source_selector.py",
        REPO_ROOT / "tests" / "test_face_reaction_source_selector_smoke.py",
    ]
...
def test_new_face_reaction_source_files_end_with_newline():
    files = [
        REPO_ROOT / "models" / "face_reaction_source.py",
        REPO_ROOT / "core" / "face_reaction_source_selector.py",
        REPO_ROOT / "tests" / "test_face_reaction_source_selector_smoke.py",
