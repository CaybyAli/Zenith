...
def test_voice_aware_gain_loud_voice_is_lower_than_quiet_voice():
    ...
    assert loud["final_gain_db"] == -40.0
    assert -38.0 <= quiet["final_gain_db"] <= -35.0
...
def test_gain_clamp_keeps_all_final_gains_inside_owner_range():
    ...
    assert gains
    assert all(-40.0 <= gain <= -35.0 for gain in gains)
    assert plan["automation_all_final_gains_between_minus_40_and_minus_35"] is True


def test_smoothing_limits_gain_jumps_to_2db():
    windows = [
        {"start_sec": 0.0, "end_sec": 5.0, "raw_gain_db": -40.0, "final_gain_db": -40.0},
        {"start_sec": 5.0, "end_sec": 10.0, "raw_gain_db": -35.0, "final_gain_db": -35.0},
        {"start_sec": 10.0, "end_sec": 15.0, "raw_gain_db": -40.0, "final_gain_db": -40.0},
    ]
